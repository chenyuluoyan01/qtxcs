          ...
          <li class="login-right">
            <ul>
              <li class="btn-list">
                ...
                <input v-model='params.userPhone' class="text-input" type="tel" id="phone" placeholder="请输入手机号">
              </li>
              <li class="btn-list via">
                <label for="via">验证码：</label>
                <input v-model='params.imgCode' class="text-input" id="via" type="text" placeholder="验证码">
                <div class="code">
              ...
              <li class="btn-list msg-code">
                ...
                <input class="text-input" id="code" type="text" v-model='params.verificationCode' placeholder="请输入手机验证码">
              ...
              <!-- <li class="btn-list">
                ...
                <input class="text-input" type="text" id="name" placeholder="请输入昵称">
              </li> -->
              <li style="border:0">
...
export default {
  ...
  methods:{
    ...
    getCode: function(){
      let _this = this;
      let userPhone = this.params.userPhone;
      let imgCode = this.params.imgCode;
      if(userPhone == ''){
        this.$Message.warning('手机号码为空！');
        return false
      }else if(imgCode == ''){
        this.$Message.warning('图形验证码为空！');
        return false
      };
      this.$base.ajax({
        url:Api.smsGet,
        data:{userPhone:userPhone,imgCode:imgCode}
      },_this).then((res)=>{
        this.codeOpen = "close";
    ...
    logins: function(){
      let phone = this.params.userPhone;
      let password = this.params.password;
      let verificationCode = this.params.verificationCode;
      let _this = this;
      if(phone == ''){
        this.$Message.warning('手机号码为空！');
        return false
      ...
      this.$base.ajax({
        ...
        data:{phone:phone,password:password,verificationCode:verificationCode}
      },_this).then((res)=>{
