<template>
  <div style="padding-top:20px;">
    <h2 class="company" style="color:#ec681e">晴天修车商家版系统</h2>
...
import md5 from '../../../libs/core/utils/lib/md5str.js'
export default {
  name:'login',
  data: function() {
    return {
  ...
  methods:{
    ...
    login: function(){
      ...
      if(phone == ''){
        ...
        data.sunnyfix_sign = md5(Api.login,data);
        $.post(Api.login,data,function(res){
          if(res.code == 0){
            localStorage.setItem("token",res.object.token);
            localStorage.setItem("userPhone",res.object.phone);
            localStorage.setItem("factoryId",res.object.factoryId || 59);
            _this.$Message.success('登录成功！');
            _this.utils.upBrandMap(_this);
            _this.$router.push({
                path: 'storeMeg/storeSets'
            });
          }else{
